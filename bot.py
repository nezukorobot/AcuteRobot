...
def main():
    bot.add_handler(MessageHandler(anime, filters.regex(r'search')), group=1)
    bot.add_handler(MessageHandler(start_message, filters.command('favorite')), group=2)
    bot.add_handler(MessageHandler(dev_info, filters.command('inlinequery')), group=3)
    bot.add_handler(MessageHandler(genres, filters.command('lyrics')), group=4)
